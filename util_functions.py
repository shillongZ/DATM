...
def load_data_set(dataset):
    ...
    csd_file = os.path.join(filepath, 'CSD/{}_text_csds.txt'.format(dataset)) # or 'CSD/{}_label_csds.txt'
...
def get_acc_basic(predict, label,model):
    ...
    if model=='test':
        print(predict, torch.unique(predict).size(0))
...
def symmetric_normalize_adj(adj):
    ...
    return adj.dot(d_mat_inv_sqrt).transpose().dot(d_mat_inv_sqrt).tocoo(), edge_index, edge_weight
...
def min_max_normalize(tensor):
    ...
    _, n = tensor.shape
    range[range==0] = n
    normalized = (tensor - min_val) / range
    return normalized
...
def calculate_accuracy_train(features, labels, class_emb):
    with torch.no_grad():
        similarity_matrix = features @ class_emb.T
        predictions = similarity_matrix.argmax(dim=1)
        correct = (predictions == labels).float().sum()
        accuracy = correct / labels.size(0)
        # print('right', correct)
        # print('label', labels)
        # print('pre', predictions, torch.unique(predictions).size(0))
        # print('acc', accuracy)
        return accuracy
    
def calculate_accuracy_test(features, labels, class_emb, num_of_train):
    with torch.no_grad():
        class_emb = class_emb[num_of_train:]
        similarity_matrix = features @ class_emb.T
        predictions = similarity_matrix.argmax(dim=1) + num_of_train
        correct = (predictions == labels).float().sum()
        accuracy = correct / labels.size(0)
        # print('right', correct)
        # print('label', labels)
